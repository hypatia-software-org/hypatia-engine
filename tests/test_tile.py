...
class TestTile:
    ...
    def test_animated_tile(self):
        ...
        tilesheet = Tilesheet(tilesheet_surface, 1, 1)
        tile = Tile(tilesheet, 1, TileFlags.ANIMATED, {"animation": [[1, 1], [2, 1]]})

        # test first update, color should be 0, 255, 0 (tile 1 in given tilesheet)
        tile.update(0)

        test_surface_one = pygame.Surface((1, 1), pygame.SRCALPHA)
        test_surface_one.fill((0, 255, 0))

        assert compare_surfaces(tile.image, test_surface_one)

        # test second update, color should be 0, 0, 255 (tile 2 in given tilesheet)
        tile.update(1)

        test_surface_two = pygame.Surface((1, 1), pygame.SRCALPHA)
        test_surface_two.fill((0, 0, 255))

        assert compare_surfaces(tile.image, test_surface_two)